...
def show_weapon_statistics():
   weapon_list = (db_session.query(Weapon.weapon_name.label('weapon_name'),func.count(Weapon.weapon_name).label('count'))
                  .join(Weapon.kills)
                  .filter(Kill.player_killer_id != Kill.player_killed_id)
                  .group_by(Weapon.weapon_name)
                  .subquery())
   kill_weapon_player = (db_session.query(Kill.weapon_id.label('weapon_id'),
                        ...
                        .subquery())
   best_kill_weapon = (db_session.query(kill_weapon_player.c.weapon_id.label('weapon_id'),
                                          func.max(kill_weapon_player.c.count).label('maxi'))
                       .group_by(kill_weapon_player.c.weapon_id)
                       .subquery())

   best_player_weapon = (db_session.query(best_kill_weapon.c.maxi.label('kill'),
   ...
   return render_template('weapons.html', best_player_weapon = best_player_weapon)
